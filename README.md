...
[观看操作演示（MP4，35 秒）](docs/demo.mp4) · [在线体验](https://snap-stock-one.vercel.app/)

![Snap Stock 线上完整分析：价格图表、交叉事件、程序计算与 DeepSeek 解读](docs/screenshot.png)

图片与视频来自线上 AAPL 实际分析页面，行情截至 2026-09-09，非实时行情。视频由实际操作画面按步骤剪辑，无音频，非连续录屏；包含信号选择、时间范围切换和解读展示。
...
1. 导入仓库根目录，使用 Node.js 22+，构建命令和输出目录保持默认。
2. 按需添加 `DEEPSEEK_API_KEY`，启用支持 90 秒执行时长的 Fluid Compute；其余配置见 `vercel.json`。
3. 部署后访问 `/health` 确认服务正常。云端不要填写 `localhost` / `127.0.0.1` 代理；行情源仍可能限制云服务器 IP。

公开演示请配置访问保护或限流，并限制模型费用；内存并发限制不等于全局限流。
